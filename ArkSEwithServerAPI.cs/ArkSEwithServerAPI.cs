...
{
    public class ArkSEwithServerAPI : SteamCMDAgent
    {
        // - Plugin Details
        public Plugin Plugin = new Plugin
        {
            name = "WindowsGSM.ArkSEwithServerAPI", // WindowsGSM.XXXX
            author = "ohmcodes™",
            description = "WindowsGSM plugin for supporting ARK:SE™ Dedicated Server with ServerAPI",
            version = "1.0.0",
            url = "https://github.com/ohmcodes/WindowsGSM.ArkSEwithServerAPI", // Github repository link (Best practice)
            color = "#008B8B" // Color Hex
        };

        // - Standard Constructor and properties
        public ArkSEwithServerAPI(ServerConfig serverData) : base(serverData) => base.serverData = _serverData = serverData;
		...
		private string apiUrl = $"https://api.github.com/repos/ArkServerApi/AseApi/releases/latest";
        private string serverAPIFileName = "ArkApi_3.56.zip";

        private List<string> filesToDelete;

        // - Settings properties for SteamCMD installer
        public override bool loginAnonymous => true; 
        public override string AppId => "376030"; /* taken via https://steamdb.info/app/376030/info/ */

        // - Game server Fixed variables
        public string StartPath = @"ShooterGame\Binaries\Win64\ShooterGameServer.exe"; // Game server start path
        public string FullName = "ARK:SE™ Dedicated Server with ServerAPI"; // Game server FullName
        ...
        public string ServerName = "wgsm_arkse_serverapi_dedicated";
        ...
        public string Additional = "?RCONEnabled=True?RCONPort=31001?ServerAutoForceRespawnWildDinosInterval=86400?AllowCrateSpawnsOnTopOfStructures=True?ClampItemStats=true -UseBattlEye -forcerespawndinos -servergamelog -servergamelogincludetribelogs -ServerRCONOutputTribeLogs -d3d10 -sm4 -lowmemory -nomansky -nosound -PublicIPForEpic=216.232.207.36 -nosteamclient -game -server -log -MinimumTimeBetweenInventoryRetrieval=3600 -ActiveEvent=CHANGE_OR_REMOVE -EnableIdlePlayerKick -clusterid=CHANGE_ME -ClusterDirOverride=\"C:\\ARK_ASM\\ASM_Data\\Vanilla\""; // Additional server start parameter
        ...
        public async Task<Process> Start()
        {
            ...
            param.Append(!string.IsNullOrWhiteSpace(_serverData.ServerMap)? _serverData.ServerMap : "TheIsland");

            param.Append("?listen");

            param.Append(!string.IsNullOrWhiteSpace(_serverData.ServerName)? $"?SessionName=\"{_serverData.ServerName}\"" : "SessionName=\"wgsm_arkse_serverapi_dedicated\"");

            param.Append(!string.IsNullOrWhiteSpace(_serverData.ServerIP)? $"?PublicIPForEpic={_serverData.ServerIP}" : _serverData.GetIPAddress());
        ...
        private async void DownloadServerAPI()
        {
            ...
            try
            {
				// https://github.com/ServersHub/ServerAPI/releases/latest
